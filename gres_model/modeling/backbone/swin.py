...
class MMBasicLayer(nn.Module):
    def __init__(
        ...
        self.fusion = PWAM(
            dim,  # both the visual input and for combining, num of channels
            dim,  # v_in
            lang_dim,  # l_in
            dim,  # key
            dim,  # value
        ...
        if downsample is not None:
            ...
            self.res_gate = nn.Sequential(
                nn.Linear(dim, dim, bias=False), nn.ReLU(), nn.Linear(dim, dim, bias=False), nn.Tanh()
            )
...
class PWAM(nn.Module):
    def __init__(self, dim, v_in_channels, l_in_channels, key_channels, value_channels, num_heads=0, dropout=0.0):
        ...
        self.vis_project = nn.Sequential(
            nn.Conv1d(dim, dim, 1, 1),  # the init function sets bias to 0 if bias is True
            nn.GELU(),
            nn.Dropout(dropout),
        )
        ...
        self.project_mm = nn.Sequential(nn.Conv1d(value_channels, value_channels, 1, 1), nn.GELU(), nn.Dropout(dropout))
...
class D2SwinTransformer(MultiModalSwinTransformer, Backbone):
    def __init__(self, cfg, input_shape):
        ...
        lang_dim = cfg.REFERRING.LANG_DIM
