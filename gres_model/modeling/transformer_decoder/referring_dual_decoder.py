...
class MultiScaleMaskedDualReferringDecoder(MultiScaleMaskedReferringDecoder):
    ...
    def __init__(
        ...
        self.VisToLangLayers = nn.ModuleList()

        self.vis_to_lang_weight = vis_to_lang_weight
    ...
    def forward(
        ...
        for i in range(self.num_layers):
            ...
            if i in self.group_layers:
                ...
                # Language-Region Cross-Attention
                vis_to_lang_ca = self.VisToLangLayers[group_idx]
                lang_vis_enhanced = vis_to_lang_ca(
                    lang_feat_att.permute(1, 0, 2),  # (N_l, B, C)
                    prev_query_output  # (Q, B, C)
                )
                lang_vis_enhanced = lang_vis_enhanced.permute(1, 0, 2)  # (B, N_l, C)
                lang_feat_att = lang_feat_att + lang_vis_enhanced * self.vis_to_lang_weight
