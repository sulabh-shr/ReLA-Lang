...
class MultiScaleMaskedLangSimpleReferringDecoder(MultiScaleMaskedReferringDecoder):
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer in self.group_layers:
                ...
                decoder = nn.TransformerDecoder(
                    decoder_layer,
                    num_layers=group_depths[group_idx],
                )
                self.LangGroupLayers.append(decoder)
                num_group_tokens = group_tokens[group_idx]
                self.group_tokens.append(nn.Embedding(num_group_tokens, hidden_dim))
                self.group_pos.append(nn.Embedding(num_group_tokens, hidden_dim))
                group_idx += 1
    ...
    def forward(self, x: List[torch.Tensor], mask_features: torch.Tensor, lang_feat: torch.Tensor, mask=None):
        ...
        for i in range(self.num_layers):
            ...
            if i in self.group_layers:
                grouping_layer = self.LangGroupLayers[group_idx]
                group_tokens = self.group_tokens[group_idx].weight  # (G_l, C)
                group_tokens = group_tokens.unsqueeze(0).repeat(bs, 1, 1)  # (B, G_l, C)
                group_pos = self.group_pos[group_idx].weight
                group_pos = group_pos.unsqueeze(0).repeat(bs, 1, 1)  # (B, G_l, C)
                group_tokens = group_tokens + group_pos
                lang_feat_att = grouping_layer(  # (B, G_l, C)
                    tgt=group_tokens,  # (B, G_l, C)
                    memory=lang_feat_att,  # (B, G_(l-1), C)
                )
                group_idx += 1
